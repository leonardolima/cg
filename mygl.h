...
Pixel interpolate(Pixel p1, Pixel p2, Pixel p)
{
	...
	if (abs(dy) <= dx)
	{
		p.c.r = (unsigned char) (p1.c.r + ((p2.c.r - p1.c.r) * ((p.x - p1.x)/dx)));
		p.c.g = (unsigned char) (p1.c.g + ((p2.c.g - p1.c.g) * ((p.x - p1.x)/dx)));
		p.c.b = (unsigned char) (p1.c.b + ((p2.c.b - p1.c.b) * ((p.x - p1.x)/dx)));
		std::cout << "p.c.r = " << (int)p.c.r << "  | p.c.g = " << (int)p.c.g << "\n";
	} else {
		p.c.r = (unsigned char) (p1.c.r + ((p2.c.r - p1.c.r) * ((p.y - p1.y)/dy)));
		p.c.g = (unsigned char) (p1.c.g + ((p2.c.g - p1.c.g) * ((p.y - p1.y)/dy)));
		p.c.b = (unsigned char) (p1.c.b + ((p2.c.b - p1.c.b) * ((p.y - p1.y)/dy)));
		std::cout << "p.c.r = " << (int)p.c.r << "  | p.c.g = " << (int)p.c.g << "\n";
	}
	return p;
}
